...
const COOKIE_OPTS = 'HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=31536000';

function key() {
  return crypto.createHash('sha256')
    .update(process.env.COOKIE_SECRET || 'dev-secret-change-me')
    .digest();
}
...
function setSession(res, session) {
  const cookie = `${COOKIE}=${encrypt(session)}; ${COOKIE_OPTS}`;
